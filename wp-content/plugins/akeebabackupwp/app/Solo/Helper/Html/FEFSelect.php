...
class FEFSelect extends AbstractHelper
{
	 ...
	 * @see     JFormFieldRadio
	 */
	public function booleanlist($name, $attribs = [], $selected = null, $yes = 'AWF_YES', $no = 'AWF_NO', $id = false)
	{
		$arr = [
			$this->option('0', Text::_($no)),
			$this->option('1', Text::_($yes)),
		];
	...
	{
		// Set default options
		$options = array_merge($this->getContainer()->html->getFormatOptions(), ['format.depth' => 0, 'id' => false]);
		...
		$html       = $baseIndent . '<select' . ($id !== '' ? ' id="' . $id . '"' : '') . ' name="' . $name . '" '
		              . $attribs . '>' . $options['format.eol'] . $this->options($data, $options) . $baseIndent
	 ...
	 */
	public function groupedlist($data, $name, $options = [])
	{
		...
		foreach ($data as $dataKey => $group)
		{
			...
			else
			{
				$html .= $groupIndent . '<optgroup' . (empty($id) ? '' : ' id="' . $id . '"') . ' label="'
				         ...
				         . '">' . $options['format.eol'] . $this->options($subList, $options) . $groupIndent
	...
	{
		// Set default options
		$options = array_merge($this->getContainer()->html->getFormatOptions(), ['format.depth' => 0, 'option.format' => '', 'id' => null]);
	...
	{
		$options = array_merge(
			$this->getContainer()->html->getFormatOptions(), $this->optionDefaults['option'],
		...
		foreach ($arr as $elementKey => &$element)
		{
			...
			{
				$html       .= $baseIndent . '<optgroup label="' . ($options['list.translate'] ? Text::_($text) : $text)
				               . '">' . $options['format.eol'];
				$baseIndent = str_repeat($options['format.indent'], ++$options['format.depth']);
			}
	...
	)
	{
		...
		foreach ($data as $optionObject)
		{
			...
			if ($forToggle)
			{
				$html .= "\n\t" . '<input type="radio" name="' . $name . '" id="' . $id . '" value="' . $optionValue
				         . '" ' . $extra . $attribs . ' />';


				$html .= "\n\t" . '<label for="' . $id . '" id="' . $id . '-lbl" ' . $labelAttributes . '>' . $labelText
				         . '</label>';
			}
			else
			{
